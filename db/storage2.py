...
def upload_local_image_to_storage(filepath: str, content_id: str, index: int, floor:str, bucket: str = "dmm-images2") -> str:
    ...
    try:
        ...
        # すでに存在する場合はスキップ
        files = supabase2.storage.from_(bucket).list(f"{floor}/{content_id}/")
        ...
        with open(filepath, "rb") as f:
            supabase2.storage.from_(bucket).upload(
                path=storage_path,
                file=f,
...
def upload_image_to_storage(url: str, content_id: str, index: int, bucket: str = "dmm-images2") -> str:
    try:
        ...
        # 既存チェック
        files = supabase2.storage.from_(bucket).list(f"{content_id}/")
        if any(file["name"] == filename for file in files):
            logging.info("[SKIP] 既に存在: %s", filename)
        else:
            # アップロード
        ...
        # 公開URLを取得
        public_url = supabase2.storage.from_(bucket).get_public_url(storage_path).get("publicUrl")
        return public_url
